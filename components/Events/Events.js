...
class Events extends Component {
    ...
    calcEventStyle = (item) => {
        ...
        const height =
            (moment(item.endTime).diff(item.startTime, "minutes") *
                CONTENT_HEIGHT) /
            MINUTES_IN_DAY;
            ...
            return events.reduce((eventsAcc, event, i) => {
                ...
                for (let j = 0; j < i; j += 1) {
                    ...
                    const foundDuplicate =
                        ...
                        previousEvent.style.top + previousEvent.style.height >
                            style.top;
                    if (foundDuplicate) {
                        numberOfDuplicate += 1;
                        style.left = 5 + itemWidth / numberOfDuplicate;
                        style.width = itemWidth / numberOfDuplicate;
                        previousEvent.style.width =
                            itemWidth / numberOfDuplicate;
                    }
                }
                eventsAcc.push({
                    data: event,
    ...
    getEventItemWidth = () => {
        ...
        return (EVENTS_CONTAINER_WIDTH - 32) / nDays;
    };
    ...
    render() {
        ...
        let totalEvents = this.catEventsByDays(
            nDays,
            sortedEvents,
            selectedDate
        );
